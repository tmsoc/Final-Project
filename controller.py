...
class Controller:
    ...
    def add_menu_press(self):
        ...
        rest_list = self.model.restaurants_select_all()
        address = self.view.entry_rest_address.get()
        for each in rest_list:
            if address == rest_list['address']:
                id = rest_list['id']
                uploaded_menu = self._get_user_file_open_path()
                if self._verify_pdf(uploaded_menu):
                    self.import_menu(rest_id, import_file)
                ...
                "Please save the information on the screen first")
    ...
    def save_new_rest_press(self):
        ...
        confirmed = self.model.restaurant_insert(restaurant_info)
        if confirmed:
            message = "Your restaruant has been saved"
            messagebox.showinfo(
                message=message, icon="info", title="Confirmed",
            )
        else:
            message = ("Unfortunately the information could not be saved. " +
                "Please try again.")
